...
// 1. 复制文件 - 无需包， 插件 - 考试
gulp.task('copyfile', () => {
    gulp.src('src/font/*') //fonts目录下面的所有文件
        .pipe(gulp.dest('dist/src/font')); //输出的路径
});
gulp.task('copyphp', () => {
    gulp.src('php/*') //fonts目录下面的所有文件
        .pipe(gulp.dest('dist/php')); //输出的路径
});
gulp.task('copydata', () => {
    gulp.src('database/*') //fonts目录下面的所有文件
        .pipe(gulp.dest('dist/database')); //输出的路径
});
